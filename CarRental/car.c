...
int checkUserExist(User u,char*usertype)
{
    if((strcmp(u.userid,"")==0) || (strcmp(u.pwd,"")==0))
    {
        gotoxy(28,20);
        textcolor(LIGHTRED);
        printf("BOTH FIELD ARE MANDATORY");
        _getch();
        gotoxy(28,20);
        printf("\t\t\t\t\t\t\t\t\t");
        return 0;
    ...
    if(!(strcmp(usertype,"admin")))
        ...
        else if(!(strcmp(usertype,"emp")))
        {
           FILE*fp=fopen("emp.bin","rb");
            ...
            fclose(fp);
            return 1;  //code for reading emp.bin
        }
...
int empMenu()
{
    ...
    gotoxy(32,10);
    printf("3.Available Cars Details");
    gotoxy(32,11);
    printf("4.Show All Cars Details");
    gotoxy(32,12);
    printf("5.Exit");
    gotoxy(32,15);
    printf("Enter choice:");
...
void bookedCarDetails()
{
    ...
    for(i=1; i<80; i++)
        printf("%c",247);
        gotoxy(1,8);
        printf("Model\t Cust Name\t Pick Up\t Drop\t\t s_Date\t   E-Date");
        gotoxy(1,9);
        textcolor(LIGHTGREEN);
        for(i=1; i<80; i++)
            printf("%c",247);
